#!/usr/bin/env python3
"""
SkiSafe — reader-complete_final.py
...
Sessions:
    A new session is opened when reader-complete_final.py starts, or when no packet
...
Run: python3 ~/skisafe/reader-complete_final.py
...
def get_or_create_session(conn, skier_id):
    ...
    Opens a new session when:
      - reader-complete_final.py just started (no session yet), OR
...
# ── Main loop ─────────────────────────────────────────────────────────────────
def main():
    print('SkiSafe reader-complete_final.py starting')
    ...
    try:
        ...
        print('\nreader-complete_final.py stopped.')
    finally:
        try:
            ser.close()
        except Exception:
